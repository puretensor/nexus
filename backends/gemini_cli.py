"""Gemini CLI backend — subprocess stub, same pattern as claude_code."""
...
class GeminiCLIBackend:
    """Backend that shells out to the Gemini CLI (stub).

    This is a placeholder for when Google ships an agentic CLI
    comparable to Claude Code. The interface mirrors ClaudeCodeBackend.
    """
    ...
    def supports_streaming(self) -> bool:
        return False  # TBD when Gemini CLI ships
    ...
    def call_sync(
        ...
        *,
        ...
        """Synchronous Gemini CLI call (stub)."""
        cmd = [self._bin, "--prompt", prompt]

        log.info("Gemini CLI call (sync): %s", " ".join(cmd[:4]) + " ...")
