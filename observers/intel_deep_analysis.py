...
<script>window.dataLayer=window.dataLayer||[];function gtag(){{dataLayer.push(arguments);}};gtag("js",new Date());gtag("config","{bc['ga_id']}");</script>
    ...
    <style>
        :root {{ --bg-primary: #0A0C10; --bg-surface: #0F1218; --bg-elevated: #161B24; {bc['accent_css']} --text-primary: #E6EAF0; --text-secondary: #8B95A5; --text-muted: #4A5568; --border: rgba(255,255,255,0.06); --font-serif: 'Cormorant Garamond', Georgia, serif; --font-sans: 'Inter', -apple-system, sans-serif; --font-mono: 'JetBrains Mono', monospace; }}
        *,*::before,*::after {{ margin:0; padding:0; box-sizing:border-box; }}
        ...
        nav {{ position:fixed; top:0; left:0; right:0; z-index:1000; padding:1.25rem calc(max(2rem, (100% - 1140px) / 2)); display:flex; align-items:center; justify-content:space-between; background:rgba(10,12,16,0.85); backdrop-filter:blur(20px); border-bottom:1px solid var(--border); }}
